...
function dateParser(date) {
  const base = date.split('-');
  const baseHour = base[2].split('T');

  const fixedHour = baseHour[1].split('.')[0];
  const day = baseHour[0];
  const month = base[1];
  const year = base[0];

  return(`${day}/${month}/${year} - ${fixedHour}`);
}
      ...
      <div className="hcard-info">
        ...
        <div className="hcard-details">
          {favOrDone === 'done' && (
            <p>
              <b>{`Feita em: `}</b>
              <span data-testid={`${index}-horizontal-done-date`}>{dateParser(card.doneDate)}</span>
            </p>
          )}
          {favOrDone === 'done' && (
            <span>
              {card.tags
                ...
                .map((each) => (
                  <CardButton className="h-tags" data-testid={`${index}-${each}-horizontal-tag`}>
                    {each}
                  </CardButton>
                ))}
            </span>
          ...
          <div>
            {favOrDone === 'fav' && (
              <div className="hcard-btns">
                ...
                <CardBody
                  className="det-btn"
                  onClick={() => shareBt(card.id, card.type, setCopiado)}
                >
                  <img
